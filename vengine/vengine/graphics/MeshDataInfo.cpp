...
bool MeshDataInfo::areStreamsValid(const VertexStreams& vertexStreams)
{
    ...
    maxVerts = std::max(maxVerts, vertexStreams.normals.size());
    ...
    if (vertexStreams.normals.size() > 0 && vertexStreams.normals.size() != maxVerts) valid = false;
...
size_t MeshDataInfo::getVertexSize(const VertexStreams& vertexStreams)
{
    ...
    if (vertexStreams.normals.size() > 0) vertexSize += sizeof(vertexStreams.normals[0]);
