...
Collider ColliderLoader::makeCollisionShape(const ColType& type, const aiMesh* mesh)
{
    ...
    {
        glm::vec3 whd(abs(mesh->mVertices[0].x), abs(mesh->mVertices[0].y), abs(mesh->mVertices[0].z));
    ...
    while (!nodeStack.empty())
    {
        ...
        for (size_t j = 0; j < node->mNumMeshes; j++)
        {
            ...
            if (meshName.length() > 7 && meshName.substr(0, 8) == "Collider")
            {
                ...
                //eRotation.z += 180;  //assimp weirdly makes mesh.rot.x -90 as def
...
)
{
    for (int i = 0; i < colliders.size(); i++)
    {
        ...
        Collider colliderCopy = colliders[i].col;
