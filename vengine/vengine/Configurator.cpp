...
{
    void readConfIntoMap(std::ifstream &conf_file, std::map<std::string, std::string> &configs) 
    {
        std::string line;
    ...
    void storeConfInMemory(std::map<std::string, std::string> &configs) 
    {
        for (auto &rule : conf.rules) 
        {
            ...
            if (configs.end() != map_rule) 
            {
                ...
                {
                case Type::int_t:
                    rule_val->value = std::any_cast<int>(std::stoi(std::string(map_rule->second)));
                    break;
                case Type::float_t:
                    rule_val->value = std::any_cast<float>(std::stof(std::string(map_rule->second)));
                    break;
                case Type::douible_t:
                    rule_val->value = std::any_cast<double>(std::stod(std::string(map_rule->second)));
                    break;
                case Type::string_t:
    ...
    void verifyStoredConfigs(std::map<std::string, std::string> &configs) 
    {
        if (configs.size() != conf.rules.size()) 
        {
            for (auto &rule_config : configs) 
            {
                ...
                {
                    std::cout << "config.cfg invalid option: " << rule_config.first << std::endl;
                }
            }
            for (auto &rule : conf.rules) 
            {
    ...
    void loadConfIntoMemory() 
    {
        ...
        std::ifstream conf_file("vengine_assets/config.cfg", std::ios::in);
